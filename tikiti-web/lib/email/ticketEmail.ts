...
export async function sendTicketEmail(bookingId: string, booking: any, ticketUrl?: string): Promise<'sent' | 'skipped' | 'failed'> {
  ...
  <div style="background:#faf9f2;padding:32px 16px;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#202220">
    <div style="max-width:480px;margin:0 auto">
      ...
      ${ticketUrl ? `<div style="text-align:center;margin:0 0 20px"><a href="${escapeHtml(ticketUrl)}" style="display:inline-block;background:#f44929;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:14px 28px;border-radius:999px">Open my ticket</a></div>` : ''}
      ...
      <p style="font-size:12px;color:#65675d;margin-top:20px">Your ticket also lives in the Tickets tab of the Tikiti app. Please don’t share this QR code.</p>
    </div>
  </div>`;
